...
export const getWhatsNew = async (req, res) => {
    const {  Product, Page, PageSize } = req.query;
    ...
    try {
        let query = `SELECT * FROM Whats_New WHERE 1=1`;
        let countQuery = `SELECT COUNT(*) as totalCount FROM Whats_New WHERE 1=1`;

        const replacements = {};

        if (Product) {
            query += " AND Product = :Product";
            countQuery += " AND Product = :Product";
